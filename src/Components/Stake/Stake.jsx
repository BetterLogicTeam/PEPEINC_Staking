...
import { Token_Abi, Token_Contract, staking, staking_Abi } from "../../Contracts/contract";
...
function Stake() {
  ...
  const Stake = async () => {
    try {
      if (getValue == "") {
        ...
        await ContractOfToken.methods
          .approve(staking, value.toString())
          .send({
            from: acc,
          });
  ...
  const Claim = async () => {
    try {
     ...
     let claimable= await stakingContractOf.methods.pendindRewards(acc).call();
